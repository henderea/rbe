...
global.helpers[:print_cmd] =->(sudo, cmd, cmd_args, args) {
  ...
  Rbe::IO.puts "> #{sudo.nil? ? '' : "#{sudo} "}#{clean_cmd(cmd)} #{arr.join(' ')}"
}
...
global.helpers[:print_list] =->(cmd_id, indent = 0, lc = false) {
  ...
  if cmds.nil? || cmds.empty?
    Rbe::IO.puts "#{' ' * indent}Did not find any commands matching #{cmd_id}"
  else
    ...
    cmds.each { |cmd|
      ...
      if info.command.is_a?(Array)
        Rbe::IO.puts "#{' ' * indent}#{clean_cmd(cmd.to_s).ljust(longest_cmd + 2)}=> [\n"
        ...
        Rbe::IO.puts "#{' ' * indent}#{' ' * (longest_cmd + 4)} ]"
      else
        Rbe::IO.puts "#{' ' * indent}#{clean_cmd(cmd.to_s).ljust(longest_cmd + 2)}=> #{info.silent ? '(silent) ' : ''}#{info.sudo.nil? ? '' : "#{info.sudo} "}#{subs_vars([info.command], [], true).first.first} #{array_to_args(info.args, []).join(' ')}"
      end
